...
?>
<main style="padding-bottom: 500px;padding-left: 10%;padding-right: 10%;" >  
<button class="btn btn-secondary active" onclick="clickGetVactions()">获取假期数据</button>
                ...
                success: function (result) {//result类型为String
                    ...
                    if (status == 1) {
                        for (i = 0; i < data.length; i++) {
                            ...
                            //alert(status_id);
                            if (status == 0) {
                                $str_extra = "<td><div id='" + div_id + "'><button class=' btn btn-secondary active ' onclick=\"clickPass('" + div_id + "','" + id + "','" + status_id + "','" + 1 + "')\">同意</button><button onclick=\"clickPass('" + div_id + "','" + id + "','" + status_id + "','" + 2 + "')\">不同意</button></div></td>" +
                                        "</tr>";
                            } else {
                                $str_extra = "<td></td>" +
                                        "</tr>";
...
<script>
    function clickPass(div_id, vacation_id, status_id, method) {
        switch (method) {
            case "1":
                connectToServcie(vacation_id, 1, status_id, div_id);
                break;
            case "2":
                connectToServcie(vacation_id, -1, status_id, div_id);
                break;
    ...
    function connectToServcie(vacation_id, status, status_id, div_id) {
        $.ajax({
            type: "Post",
            url: GlobalConfig.VacationUrl,
